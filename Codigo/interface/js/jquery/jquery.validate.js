...
$.extend($.validator, {
	...
	messages: {
		...
		maxlength: $.format("&nbsp;Erro! M&aacute;ximo de {0} caracteres."),
		minlength: $.format("&nbsp;Erro! M&iacute;nimo de {0} caracteres."),
	...
	prototype: {
		...
		resetForm: function() {
			...
			this.elements().removeClass( this.settings.borderErrorClass );
		},
...
jQuery.validator.addMethod("cnpj", function(cnpj, element) {
   ...
   while( cnpj.indexOf("_") > -1 ) cnpj = cnpj.replace('_','');

   var numeros, digitos, soma, i, resultado, pos, tamanho, digitos_iguais;
      
   digitos_iguais = 1;

    $(element).removeClass( this.settings.borderErrorClass );
 	if (cnpj.length == 0) return true;
	
	if (cnpj.length < 14){
		$(element).addClass( this.settings.borderErrorClass );
		return false;
	}
...
}, "Informe um e-mail válido."); // Mensagem padrão
...
jQuery.validator.addMethod("fone", function(fone, element) {
    fone = jQuery.trim(fone);
    ...
    while( fone.indexOf("_") > -1 ) fone = fone.replace('_','');

    $(element).removeClass( this.settings.borderErrorClass );

    if (fone.length == 0) return true;
...
}, "Informe um fone vÃ¡lido."); // Mensagem padrÃ£o

jQuery.validator.addMethod("nome", function(nome, element,param) {
    nome = jQuery.trim(nome);
    $(element).removeClass( this.settings.borderErrorClass );

    if (nome.length < param){
        $(element).addClass( this.settings.borderErrorClass );
        return false;
    }else{
        return true;
...
}, "Informe um nome vÃ¡lido."); // Mensagem padrÃ£o
